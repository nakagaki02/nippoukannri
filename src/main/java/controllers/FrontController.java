...
public class FrontController extends HttpServlet {
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //該当するActionオブジェクトを作成(例：リクエストパラメータ action = Employeeの場合、actions.EmployeeActionオブジェクト)
            type = Class.forName(String.format("actions.%sAction", actionString));

            //ActionBaseのオブジェクトにキャスト（例：actions,EmployeeActinオブジェクト→actions.ActionsBaseオブジェクト）
        ...
        //リクエストパラメータに設定されている"action"の値が不正の場合（例 action=xxxxなど該当するActionクラスがない場合）
